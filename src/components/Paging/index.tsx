...
const Paging: FC<PagingComponentPropType> = props => {
  ...
  const last = (page: number = 20) => <div className={ checkIndex === (total / page) ? 'active' : '' } onClick={ () => onChangeComments(total / page ) }>{ Math.floor(total / page) }</div>;
          ...
          <>
            { firstShow ? <>{ first() }...</> : first() }
            {
              nums.map((num: number, index: number) => {
                return <div className={ Math.floor(num) === Math.floor(checkIndex) ? 'active' : ''  } key={ index } onClick={ () => onChangeComments(num) }>{ Math.floor(num) }</div>;
              })
            }
  ...
  return (
      <div className="paging-wrap">
        ...
        <button className="next-page" onClick={ () => onChangeComments(  checkIndex + 1) } disabled={ checkIndex >= (total / (pageIng ? pageIng : 20)) }>下一页</button>
      </div>
  );
};
